...
class _UserFollowingState extends State<UserFollowing> {
  ...
  bool loading = false;
  UserFollowing$Query$PaginatedFollow? paginatedUsers;
      ...
      body: Container(
        child: BlocBuilder<UserFollowCubit, UserFollowState>(
          builder: (context, state) {
            if (state is UserFollowingState) {
              if (state.uuid == uuid) {
                ...
                children: [
                  ...
                  ...paginatedUsers!.users.map((user) {
                    return UserFollowTile(
                      user: user,
                      key: Key("userFollowing_${widget.id}_${user.id}")
                    );
                  }).toList(),
                  Container(
                    child: BlocConsumer<LoadingUserFollowCubit, LoadingUserFollowState>(
                      listener: (context, loadingState) {
                        if (
                          ...
                          loadingState.isFollowers == false
                        ) {
                          loading = true;
                        } else if (

...
class _TrimEditorState extends State<TrimEditor> with TickerProviderStateMixin {
        ...
        if (isPlaying) {
          ...
          setState(() {
            ...
            if (_currentPosition >= _videoEndPos.toInt()) {
              videoPlayerController.seekTo(Duration(milliseconds: _videoStartPos.toInt())).then((value) {
  ...
  void _onDragStart(DragStartDetails details) {
    ...
    double spacing = (MediaQuery.of(context).size.width - _thumbnailViewerW) / 2;

    _allowDrag = true;
    
    if (
      details.globalPosition.dx <= (_startPos.dx + spacing) + (widget.sideTapSize / 2) &&
      details.globalPosition.dx >= (_startPos.dx + spacing) - (widget.sideTapSize / 2)
    ) {
      print("left");
      _dragType = EditorDragType.left;
    } else if (
      details.globalPosition.dx <= (_endPos.dx + spacing) + (widget.sideTapSize / 2) &&
      details.globalPosition.dx >= (_endPos.dx + spacing) - (widget.sideTapSize / 2)
    ) {
      print("right");

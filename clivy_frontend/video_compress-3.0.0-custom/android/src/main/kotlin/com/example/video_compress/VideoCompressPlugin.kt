    ...
    override fun onMethodCall(call: MethodCall, result: MethodChannel.Result) {
        ...
        when (call.method) {
            ...
            "compressVideo" -> {
                ...
                val fileHeight = call.argument<Int>("fileHeight")!!
                val fileWidth = call.argument<Int>("fileWidth")!!
                ...
                Log.d("AHHHHHHHHHHHHH", fileHeight.toString())
                Log.d("AHHHHHHHHHHHHH", fileWidth.toString())

                when (quality) {

                    0 -> {
                        videoTrackStrategy = DefaultVideoStrategy.Builder().addResizer(ExactResizer(fileWidth, fileHeight)).addResizer(AtMostResizer(540, 960)).frameRate(frameRate!!).build()
                    }
                    1 -> {
                        videoTrackStrategy = DefaultVideoStrategy.Builder().addResizer(ExactResizer(fileWidth, fileHeight)).addResizer(AtMostResizer(480, 640)).frameRate(frameRate!!).build()
                    }
                    2 -> {
                        videoTrackStrategy = DefaultVideoStrategy.Builder().addResizer(ExactResizer(fileWidth, fileHeight)).addResizer(AtMostResizer(540, 960)).frameRate(frameRate!!).build()
                    }
                ...
                transcodeFuture = Transcoder.into(destPath!!)
                        ...
                        .setListener(object : TranscoderListener {
                            ...
                            override fun onTranscodeFailed(exception: Throwable) {
                                Log.d("EHHHHHHHHHHHHH", exception.toString())
                                result.success(null)
                            }
                        }).transcode()
            }

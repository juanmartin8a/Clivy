...
public class SwiftVideoCompressPlugin: NSObject, FlutterPlugin {
    ...
    public func handle(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
        ...
        case "compressVideo":
            ...
            let fileHeight = args!["fileHeight"] as! Int
            let fileWidth = args!["fileWidth"] as! Int
            compressVideo(path, quality, deleteOrigin, startTime, duration, includeAudio,
                          frameRate, CGFloat(fileHeight), CGFloat(fileWidth), result)
    ...
    private func compressVideo(_ path: String,_ quality: NSNumber,_ deleteOrigin: Bool,_ startTime: Double?,
                               ...
                               _ fileHeight: CGFloat,_ fileWidth: CGFloat,
        ...
        print(duration!)
        print(minStartTime)
        print(maxDurationTime)

        let transform = sourceVideoTrack!.preferredTransform
        let size = sourceVideoTrack!.naturalSize.applying(transform)
        let realSize = CGSize(width: abs(size.width), height: abs(size.height))
        ...
        let cmCompositionStartTime = CMTimeMakeWithSeconds(0, preferredTimescale: timescale)
        let cmCompositionDurationTime = CMTimeMakeWithSeconds(videoDuration, preferredTimescale: timescale)
        let compositionTimeRange: CMTimeRange = CMTimeRangeMake(start: cmCompositionStartTime, duration: cmCompositionDurationTime)
        
        let isIncludeAudio = includeAudio != nil ? includeAudio! : true
        
        let session = getComposition(isIncludeAudio, compositionTimeRange, sourceVideoTrack!)
        
        let exporter = AVAssetExportSession(asset: session, presetName: getExportPreset(quality))!

        // let newVideTrack = avController.getTrack(session)
        ...
        if frameRate != nil {
            ...
            videoComposition.renderSize = CGSize(width: fileWidth, height: fileHeight)
            ...
            print(scale == -1)
            print(sourceVideoTrack!.naturalSize.width != realSize.width)
            ...
            if (sourceVideoTrack!.naturalSize.width != realSize.width && scale == -1) {
                let preTransform1 = CGAffineTransform(translationX: realSize.width, y: -( realSize.height - fileHeight ) / 2)
                finalTransform = preTransform1.rotated(by: CGFloat( Double.pi / 2 ))
            } else {
                // var preFinalTransform: CGAffineTransform = CGAffineTransform.identity
                finalTransform = finalTransform.translatedBy(x: -( realSize.width - fileWidth ) / 2, y: -( realSize.height - fileHeight ) / 2).concatenating(transform)//-( realSize.height - fileHeight ) / 2)
                // transform1 = CGAffineTransform(translationX: -( realSize.width - fileWidth ) / 2, y: -( realSize.height - fileHeight ) / 2).concatenating(transform)
                // finalTransform = finalTransform.translatedBy(x: realSize.width, y: 0)
                // transform1 = finalTransform
            }
            ...
            instruction.timeRange = timeRange //sourceVideoTrack!.timeRange

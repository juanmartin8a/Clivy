...
@ObjectType()
@Entity()
export class FollowNotification extends BaseEntity {
  @Field(() => Int)
  @PrimaryColumn()
  follower_id: number
  ...
  @OneToOne(() => Notification, notification => notification.followNotification, {
		onDelete: "CASCADE",
  ...
  @Field(() => User)
  @ManyToOne(() => User, user => user.followerNotification, {
		onDelete: "CASCADE",
		orphanedRowAction: 'delete'
	})
	...
	@ManyToOne(() => User, user => user.followNotification, {
		onDelete: "CASCADE",
		orphanedRowAction: 'delete'
	})
